...
parser.add_argument('-i', '--input', type = str, default="./avbuffpvs/", help = '''Path to PVS''')
parser.add_argument('-o', '--output', type = str, default="./playlists_mobile/", help = '''Path to output dir''')
parser.add_argument('-n', '--number', type = int, help = '''Number of playlists to generate''', default = 30)
parser.add_argument('-s', '--sessions', type = int, help = "Number of sessions", default = 3)
parser.add_argument('-p', '--prime', default=False, action="store_true",
...
def get_randomized_and_split(input_list, n_per_split):
  ...
  l = [ shuffled [i:i + n_per_split] for i in range(0, (args.sessions-1)*n_per_split, n_per_split)]
...
for user_id, prime in enumerate(primes):
  ...
  print("[info] writing training file for user " + str(user_id) + ": " + training_file.name)
  ...
  for session_id, playlist in zip(session_ids, playlists):
    ...
    print("[info] writing session file for user " + str(user_id) + ": " + session_file.name)
    for f in playlist:
